...
const getStructure = async () => {
  ...
  if (finishedGetStructure){
    ...
    setSubmitII(countPresentedInPNR(structure, "Проведены ИИ", "Акт ИИ на подписи", 'Акт ИИ подписан', "Проведено КО", "Акт КО на подписи", 'Акт КО подписан' ));
    
    // Подсчет по КО (Проведено КО или Акт КО на подписи)
    setSubmitKO(countPresentedInPNR(structure, "Проведено КО") + 
               countPresentedInPNR(structure, "Акт КО на подписи", 'Акт КО подписан'));
...
<Text style={{ fontSize: ts(14), color: '#1E1E1E', fontWeight: 500, marginBottom: 8, textAlign: 'right', marginRight: 5 }}>{codeCCS}</Text>
    <ScrollView >
      <View style={styles.container}>
        ...
        <View style={{paddingTop: 11}}>
          <Linechart blueQuantity={data.busyStaff} dinamic={-2} title='Персонал'/>
        </View>
      </View>
    </ScrollView>
  </View>
