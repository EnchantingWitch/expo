...
import { Alert, Dimensions, Modal, Text, TextInput, TouchableOpacity, useWindowDimensions, View } from 'react-native';
...
const Linechart = ({ blueQuantity, title, dinamic, codeCCS, accessToken }: Props) => {
  ...
  const ts = (fontSize: number) => {
   ...
   const [lineData, setLineData] = useState([
    {value: 0, dataPointText: '0', label: '30.04', dataPointColor: '#0072C8', dataPointRadius: 2.5 },
    {value: 0, dataPointText: '0', label: '07.05', dataPointColor: '#0072C8', dataPointRadius: 2.5},
    {value: 0, dataPointText: '0', label: '14.05', dataPointColor: '#0072C8', dataPointRadius: 2.5},
    {value: 0, dataPointText: '0', label: '21.05', dataPointRadius: 3, dataPointColor: '#16a34a', dataPointLabelWidth: 1},
    {value: 0, dataPointText: '0', label: '29.05', dataPointColor: '#0072C8', dataPointRadius: 2.5},
    {value: 0, dataPointText: '0', label: '05.05', dataPointColor: '#0072C8', dataPointRadius: 2.5},
    {value: 0, dataPointText: '0', label: '12.05', dataPointColor: '#0072C8', dataPointRadius: 2.5},
  ]);

  const [inputValues, setInputValues] = useState<number[]>([0, 0, 0, 0, 0, 0, 0]);
 ...
 useEffect(() => {
  ...
  useEffect(() => {
        ...
        const updatedLineData = lineData.map((item, index) => {
          if (data[index]) {
            return {
              ...
              dataPointText: data[index].personnelFact?.toString() || item.dataPointText,
        ...
        updatedLineData[3] = { // Пример: обновляем только конкретную точку
          ...updatedLineData[3],
...
const getStaff= async () => {
        try {
          const response = await fetch('https://xn----7sbpwlcifkq8d.xn--p1ai:8443/capitals/getStaff/'+codeCCS,
            {method: 'GET',
            headers: {
...
const makeNewData = () => {
  ...
  const updatedData = data.map((item, index) => ({
    ...
    date: item.date // берем дату из исходных данных
  }));
...
const updateStaff = async () => {
    ...
    try {
      let response = await fetch(`https://xn----7sbpwlcifkq8d.xn--p1ai:8443/capitals/updateStaffInf`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
        ...
        body: JSON.stringify(newData), //надо сформировать коллекцию значений
      });
        ...
        <View style={{
          ...
          <View style={{ justifyContent: 'center'}}      > 
            ...
            {lineData.map((item, index) => (
                <View key={index} style={{flexDirection: 'row', justifyContent: 'space-around', paddingVertical: 3}}>
                  <TextInput 
                    style={{ 
                      textAlign: 'center', 
                      borderColor: '#E0F2FE', 
                    ...
                    editable={false}
                    value={data?.[index]?.date ? data[index].date.replace(/\.\d{4}$/, '') : item.label}
                  />
                  <TextInput 
